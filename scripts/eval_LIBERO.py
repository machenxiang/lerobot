...
Based on: https://github.com/huggingface/lerobot/issues/1316
"""
...
def eval_libero(args: Args) -> None:
    ...
    # Load Policy - from_pretrained works for both HuggingFace hub and local paths
        ...
        for episode_idx in tqdm(
            ...
            while t < max_steps:
                try:
                    ...
                    # Map camera names to what the policy expects (LIBERO has 2 cameras, copy agentview as camera3)
                    ...
                    observation = {
                        ...
                        "observation.images.camera2": torch.from_numpy(wrist_img / 255.0)
                            .permute(2, 0, 1).to(torch.float32).to(args.device).unsqueeze(0),
                        "observation.images.camera3": torch.from_numpy(agentview_image / 255.0)
                            .permute(2, 0, 1).to(torch.float32).to(args.device).unsqueeze(0),
